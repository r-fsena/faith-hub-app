...
export default function LoginScreen() {
  ...
  const { signIn, federatedSignIn, confirmSignIn } = useAuth(); // Agora puxa o confirmSignIn

  const handleLogin = async () => {
    setLoading(true);
    
    // Passo 2: O usuário recebeu um convite e agora tá forçado a trocar a senha inicial
    if (isNewPasswordRequired) {
      ...
      try {
        ...
        alert(error.message || 'Erro ao definir nova senha');
      } finally {
        setLoading(false);
      }
      return;
    ...
    // Passo 1: Validando E-Mail e Senha
    if (!email || !password) {
    ...
    try {
      ...
      // Checa se a AWS informou que foi um login de primeiro acesso que exige criação de senha própria
      ...
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.headerTextContainer}>
          <ThemedText type="title" style={styles.title}>
            {isNewPasswordRequired ? "Atualizar Senha" : "Bem-vindo"}
          </ThemedText>
          <ThemedText style={styles.subtitle}>
            {isNewPasswordRequired ? "Por segurança, defina uma nova senha para sua conta!" : "Conecte-se com a nossa família e continue sua jornada de fé."}
          </ThemedText>
        </View>
        ...
        <View style={styles.formContainer}>
          ...
          {isNewPasswordRequired ? (
            <View style={[styles.inputContainer, { backgroundColor: inputBg, marginBottom: 24 }]}>
          ...
          >
            {loading ? (
              ...
              <ThemedText style={[styles.primaryButtonText, { color: buttonTextColor }]}>
                {isNewPasswordRequired ? "Salvar e Acessar" : "Acessar Comunidade"}
              </ThemedText>
            )}
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  keyboardView: { flex: 1 },
  container: {
    flexGrow: 1, paddingHorizontal: 28, paddingTop: Platform.OS === 'ios' ? 70 : 50,
    paddingBottom: 40, alignItems: 'center',
  },
  logoContainer: { width: '100%', alignItems: 'center', marginBottom: 32 },
  logoWrapper: {
    width: width * 0.28, height: width * 0.28, borderRadius: 24, overflow: 'hidden', backgroundColor: 'transparent',
  },
  logo: { width: '100%', height: '100%' },
  headerTextContainer: { width: '100%', marginBottom: 28, alignItems: 'center' },
  title: { fontSize: 28, fontWeight: '800', marginBottom: 8, letterSpacing: -0.5 },
  subtitle: { fontSize: 15, opacity: 0.7, textAlign: 'center', letterSpacing: 0.2 },
  formContainer: { width: '100%' },
  inputContainer: {
    flexDirection: 'row', alignItems: 'center', height: 52, borderRadius: 16, marginBottom: 14,
    paddingHorizontal: 16, borderWidth: 1, borderColor: 'transparent',
  },
  inputIcon: { marginRight: 12 },
  input: { flex: 1, height: '100%', fontSize: 15, fontWeight: '500', letterSpacing: 0.2 },
  forgotPassword: { alignSelf: 'flex-end', marginBottom: 24 },
  forgotPasswordText: { fontSize: 13, fontWeight: '600', opacity: 0.7 },
  primaryButton: {
    height: 54, borderRadius: 16, justifyContent: 'center', alignItems: 'center', shadowColor: '#000',
    shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.12, shadowRadius: 10, elevation: 4,
  },
  primaryButtonText: { fontSize: 16, fontWeight: '700', letterSpacing: 0.4 },
  dividerContainer: { flexDirection: 'row', alignItems: 'center', marginVertical: 28 },
  divider: { flex: 1, height: 1 },
  dividerText: { marginHorizontal: 16, fontSize: 13, opacity: 0.4, fontWeight: '500' },
  socialButtonsContainer: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 20, marginBottom: 36 },
  socialButton: {
  ...
  footerLink: { fontSize: 14, fontWeight: '800', textDecorationLine: 'underline' },
  creditContainer: { alignItems: 'center', marginTop: 'auto', paddingTop: 40, paddingBottom: 10 },
  creditText: { fontSize: 12, opacity: 0.5, marginBottom: 8 },
  creditLogo: { width: 90, height: 28 },
});
